...
def run_iperf_test(bandwidth_mbps=300, duration=10):
...
def terminate_gnb(instance_id):
    ...
    terminate_request = requests.post(f"{NFO_BASE}/deployments/{instance_id}/terminate/", timeout=60)
...
def ue_iperf():
    try:
        ...
        result = ue.run_iperf(bitrate=data.get('bitrate', 10), duration=data.get('duration', 20))
...
def sideload_register():
    ...
    print(f"[INFO] Validated: {len([ip for ip in validated_ips if ip['reachable']])}/{len(validated_ips)}")
...
def tests_list():
    ...
    return jsonify({
        'tests': [
            {'testId': r[0], 'testType': r[1], 'targetOdu': r[2]}
            for r in rows
        ]
    })
...
def tests_run(test_id):
    """Execute single test case with TEIV integration"""
    print(f"[DEBUG] Received request for test_id: {test_id}")
    row = db.get_test_case(test_id)
    print(f"[DEBUG] Database returned: {row}")
    if not row:
        return jsonify({'error': 'Test not found'}), 404

    file_path, target_odu_urn, _ = row
    print(f"[DEBUG] Unpacked: file_path={file_path}")

    with open(file_path) as f:
        test_case = yaml.safe_load(f)
    print(f"[DEBUG] Loaded YAML: {test_case['testId']}")
    odu = teiv.get_odu(test_case['target']['oduUrn'])
    cell = teiv.get_cell(test_case['target']['cellUrn'])
    print(f"[DEBUG] TEIV lookup: odu={odu is not None}, cell={cell is not None}")
    ...
    config = {
        ...
        'artifact_repo_branch': teiv.get_helm_branch_for_odu(test_case['target']['oduUrn']),
        'target_cluster': 'cc1397ba-b1c4-4a3e-bc8d-6af58ef53818',
        'values': {
            'config': {
                'mcc': odu_attrs.get('mcc'),
                'mnc': odu_attrs.get('mnc'),
                'tac': 1,
                'physCellId': cell_attrs.get('nRPCI'),
                'absoluteFrequencySSB': odu_attrs.get('absoluteFrequencySSB')
            }
        }
    }

    # if test_case['testType'] == 'TIMING_WINDOWS':
    #     config['values']['fhi72'] = test_case['parameters']
    if test_case['testType'] == 'TIMING_WINDOWS':
        # Build FHI72 baseline from TEIV
        fhi72_baseline = {
            'dpdk_iova_mode': 'VA',
            'file_prefix': 'fhi_72',
            'T1a_cp_dl': {'min': odu_attrs.get('T1a_cp_dl_min'), 'max': odu_attrs.get('T1a_cp_dl_max')},
            'T1a_cp_ul': {'min': odu_attrs.get('T1a_cp_ul_min'), 'max': odu_attrs.get('T1a_cp_ul_max')},
            'T1a_up': {'min': odu_attrs.get('T1a_up_min'), 'max': odu_attrs.get('T1a_up_max')},
            'Ta4': {'min': odu_attrs.get('Ta4_min'), 'max': odu_attrs.get('Ta4_max')},
            'ru_config': {'iq_width': 9, 'iq_width_prach': 9},
            'prach_config': {'eAxC_offset': 4, 'kbar': 4}
        }

        # Override with test-specific parameters
        for param, value in test_case['parameters'].items():
            fhi72_baseline[param] = value

        config['values']['fhi72'] = fhi72_baseline
        print(f"[DEBUG] FHI72 config: {fhi72_baseline}")
    elif test_case['testType'] == 'CPU_SCALING':
        config['values']['resources'] = {
            'define': True,
            'limits': {'nf': {'wr_isolcpus': test_case['parameters']['isolcpus']}},
            'requests': {'nf': {'wr_isolcpus': test_case['parameters']['isolcpus']}}
        }
    ...
    results = []
    #SIDELOAD_INSTANCE_ID = "be0cc18b-1b8e-4b58-a2bc-9f4681ac6142"
    SIDELOAD_INSTANCE_ID = test_case.get('target', {}).get('sideloadInstanceId', 'be0cc18b-1b8e-4b58-a2bc-9f4681ac6142')
    IPERF_DURATION = 10
    ...
    for run in range(test_case['execution']['runsPerCase']):
        ...
        sideload_url = f"http://{sideload_info['ip_address']}:{sideload_info['port']}"
        cpu_result = {'data': None, 'error': None}

        def fetch_cpu_background():
            """Background thread for CPU profiling"""
            try:
                print(f"[DEBUG] CPU thread: Starting profiling...")
                resp = requests.post(
                    f"{sideload_url}/perf/thread_cpu_affinity",
                    json={"duration": IPERF_DURATION + 2, "pgrep": "softmodem"},
                    timeout=IPERF_DURATION + 10
                )
                if resp.status_code == 200:
                    cpu_result['data'] = resp.json()
                    print(f"[DEBUG] CPU thread: Profiling complete")
                else:
                    cpu_result['error'] = f"HTTP {resp.status_code}"
                    print(f"[DEBUG] CPU thread: Failed with {resp.status_code}")
            except Exception as e:
                cpu_result['error'] = str(e)
                print(f"[DEBUG] CPU thread: Exception - {e}")

        # STEP 1: Start CPU profiling thread FIRST
        print(f"[DEBUG] Starting CPU profiling thread ({IPERF_DURATION + 2}s)...")
        cpu_thread = threading.Thread(target=fetch_cpu_background, daemon=True)
        cpu_thread.start()

        # STEP 2: Wait for CPU profiling to start
        time.sleep(1)

        # STEP 3: Run iperf (while CPU profiling in background)
        print(f"[DEBUG] Running iperf ({IPERF_DURATION}s) during CPU profiling...")
        bandwidth_mbps = test_case['parameters'].get('iperf_bandwidth_mbps', 300)
        ...
        # STEP 4: Wait for CPU profiling to complete
        print(f"[DEBUG] Waiting for CPU thread to finish...")
        cpu_thread.join(timeout=15)

        # Check CPU results
        cpu_data = cpu_result['data']
        if cpu_data and cpu_data.get('threads'):
            total_cpu = sum(t['avg_cpu'] for t in cpu_data['threads'])
            thread_count = len(cpu_data['threads'])
            print(f"[DEBUG] CPU complete: {thread_count} threads, total {total_cpu:.1f}%")
        elif cpu_result['error']:
            print(f"[WARN] CPU profiling error: {cpu_result['error']}")
        else:
            print(f"[WARN] No CPU data received")
        print(f"[DEBUG] UE Status {ue_status}")

        # Add right after check_ue_status:
        print(f"[DEBUG] UE status keys: {ue_status.keys() if ue_status else 'None'}")
        print(f"[DEBUG] Signal value: {ue_status.get('signal') if ue_status else 'None'}")
        ...
        results.append({
            ...
            'rsrp': ue_status['signal']['rsrp'],
            'rsrq': ue_status['signal']['rsrq'],
            'sinr': ue_status['signal']['sinr'],
            'cpu_data': cpu_data
        })
    ...
    if results:
        ...
        # STEP 2: Aggregate CPU metrics
        cpu_breakdown = {}
        cpu_runs = 0

        for r in results:
            if r.get('cpu_data') and r['cpu_data'].get('threads'):
                cpu_runs += 1
                for thread in r['cpu_data']['threads']:
                    name = thread['name']
        ...
        # STEP 4: Push to InfluxDB (AFTER avg_results is created)
        try:
            influx.write_test_result(
                test_id=test_id,
