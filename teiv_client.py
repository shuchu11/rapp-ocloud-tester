...
class TEIVClient:
    ...
    def load_cache(self):
        ...
        for item in resp.json()["items"]:
            ...
            self.cache[odu["id"]] = {"type": "ODU", "data": odu}
        ...
        for item in resp.json()["items"]:
            ...
            self.cache[cell["id"]] = {"type": "Cell", "data": cell}
        ...
        print(
            f"[TEIV] Cached {len(self.cache)} entities, {len(self.relationships)} relationships"
        )
        return len(self.cache)

    def get_odu(self, odu_urn):
        """Get ODU by URN"""
        if odu_urn in self.cache:
            return self.cache[odu_urn]["data"]

        # Fetch from TEIV
        resp = requests.get(
            f"{self.base}/domains/RAN/entity-types/ODUFunction/entities/{odu_urn}"
        )
        if resp.status_code == 200:
            data = resp.json()
            # TEIV returns: {"o-ran-smo-teiv-ran:ODUFunction": [{"id": "...", "attributes": {...}}]}
            odu = data.get("o-ran-smo-teiv-ran:ODUFunction", [{}])[0]
            return odu
        return None
    ...
    def get_helm_branch_for_odu(self, odu_urn):
        ...
        print(f"DEBUG: Deployment name {name}")
        if "pegatron" in name:
